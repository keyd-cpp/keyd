...
void descriptor_map::sort()
{
	if (mapv.empty())
		return;
	// Debloated a little
	qsort(mapv.data(), mapv.size(), sizeof(descriptor), [](const void* _a, const void* _b) {
		const auto* a = (const descriptor*)_a;
		const auto* b = (const descriptor*)_b;
		if (*a < *b)
			return -1;
		if (*a == *b)
			return 0;
...
static std::vector<uint16_t> layer_composition(struct config* config, std::string_view str)
{
	...
	qsort(arr.data(), arr.size(), 2, [](const void* _a, const void* _b) -> int {
